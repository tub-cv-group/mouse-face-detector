"""
    ...
    It does no deinterlacing, so the frames might have weird lines on the edges of moving objects.
...
def video_to_frames(input_loc, output_loc, frames_to_extract=None):
    ...
    while success:#cap.isOpened():
        if (count)%report_freq == 0: print("\tFrame {} out of approximately {}..".format(count+1, approx_video_length))
        # Write the results back to output location.
        if frames_to_extract is None or count in frames_to_extract:
           cv2.imwrite(os.path.join(output_loc, "{:05d}.png".format(count)), frame) # could add something like [cv2.IMWRITE_PNG_COMPRESSION, 0] or use .jpg to get it even smaller
    ...
    print("Done extracting frames. {} frames extracted.".format(num_for_message), "It took {:.2f} seconds.".format(time.time()-time_start))
...
if __name__=="__main__":
    ...
    argparser.add_argument('-o', "--output_dir", help="Directory where the output should go. Will be created if it doesn't exist. Default: in a folder next to the video")
    argparser.add_argument('-f', "--frames", help="List of frames (indices starting at 0) to extract. Default: all frames", nargs='+', default=None)
